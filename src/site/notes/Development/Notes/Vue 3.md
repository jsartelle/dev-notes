---
{"dg-publish":true,"dg-path":"Notes/Notes/Vue 3.md","permalink":"/notes/notes/vue-3/","tags":["language/vue"]}
---
...
- define props using the `defineProps` macro, which returns an object with all the passed prop values
    - or use [[Development/Notes/Vue 3#^04077c\|type-based declaration]]
...
- events should be documented using the `defineEmits` macro, which returns an `emit` function that can be used in the script
    - or use [[Development/Notes/Vue 3#^aa6726\|type-based declaration]]
...
- `provide` key can be a string or Symbol
    ...
    - Symbol keys also [[Development/Notes/Vue 3#^2e8115\|work better in TypeScript]]
...
- always start with `use` (like [[Development/Notes/React#Hooks\|React hooks]])
...
- it is recommended **not** to use custom directives on components - directives always apply to a component's root node and can't be forwarded (unlike [[Development/Notes/Vue 3#Fallthrough Attributes\|#Fallthrough Attributes]])
...
- components can have multiple root nodes
    - non-prop attributes must be [[Development/Notes/Vue 3#Fallthrough Attributes\|forwarded to a specific node]]
...
- `v-bind.sync` is replaced with [[Development/Notes/Vue 3#Events (emits) and v-model\|v-model]]
- [[Development/Notes/Vue 3#Provide and Inject\|#Provide and Inject]] bindings are reactive
