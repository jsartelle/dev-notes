...
- classes can be added with a string as normal, or conditionally using array or object syntax (like the `classnames` package for React)
...
- elements have readonly `clientWidth`, `clientHeight`, `offsetWidth`, `offsetHeight` bindings
    - inline elements without intrinsic dimensions can't be observed unless their `display` value is changed (see [[Development/Cheat sheets/JavaScript#ResizeObserver\|ResizeObserver]])
